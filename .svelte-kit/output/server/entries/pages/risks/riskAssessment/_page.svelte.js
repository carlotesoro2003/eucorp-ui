import { J as ensure_array_like, G as escape_html, K as attr, B as pop, z as push } from "../../../../chunks/index.js";
...
function _page($$payload, $$props) {
  ...
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    ...
    $$payload.out += `<tr><td>${escape_html(risk.rrn)}</td><td>${escape_html(risk.risk_statement)}</td><td>${escape_html(classification.find((cls) => cls.id === risk.classification)?.name || "N/A")}</td><td>${escape_html(risk.actions)}</td><td>${escape_html(risk.key_persons)}</td><td>${escape_html(risk.budget)}</td><td><button class="btn btn-primary"${attr("disabled", riskAssessments.some((ra) => ra.risk_id === risk.id), true)}>Add Assessment</button> <button class="btn btn-error">Delete</button></td></tr>`;
  }
